...
真实假人需要具备玩家级行为能力，包括背包、装备、经验、攻击、挖掘、使用、传送、区块加载等。因此核心实现不能只做客户端 NPC 包，而应创建服务端可追踪的假玩家实体。
...
- `/dummy actions <name> lookat <x> <y> <z>`：看向指定方块坐标。
...
- 幽灵模式。
- 是否加载区块。
- 区块加载范围按世界 simulation-distance 处理，用于匹配真实玩家挂机红石/实体 tick 范围。
- 是否显示在 Tab 列表。
- 名字显示格式。
